...
protected:
    struct Coordinate {
        Coordinate(size_t I, size_t J, size_t K) : i(I), j(J), k(K), inside(false) {}
        Coordinate(size_t I, size_t J, size_t K, bool Inside) : i(I), j(J), k(K), inside(Inside) {}
        size_t i;
        size_t j;
        size_t k;
        bool inside;
    ...
    public:
        ...
        Node(size_t  i, size_t j, size_t k) : coordinate(i, j, k) {}
        Coordinate coordinate;
    };

    float ComputeTentativeValue(size_t  i, size_t  j, size_t  k) {
        constexpr auto inf = std::numeric_limits<float>::max();
        // Fetch (i-1,j,k)   (returns infinity if not accepted)

        auto iter = mAccepted.find(Coordinate(i - 1, j, k));
    ...
    void UpdateTentative(size_t i, size_t j, size_t k, bool inside) {
        // Check bounds
        if (i < 0 || i > GetGrid().GetDimX() - 1 || j < 0 || j > GetGrid().GetDimY() - 1 || k < 0 ||
            k > GetGrid().GetDimZ() - 1)
            return;
        ...
        auto value = ComputeTentativeValue(i, j, k);
        // mexPrintf("Tentative value for point (%i,%i,%i) computed: %f (bandwidth: %f)\n", i,j,k,
        // value, bandwidth);

        // Check whether we're inside the narrowband
        auto insideNarrowBand = inside && value * mLS->GetDx() <= -GetGrid().GetInsideConstant() ||
     ...
     */
    float InterpolateDistance(float center, float im, float ip, float jm, float jp, float km,
                              float kp) {
...
public:
    ...
    virtual void Propagate(float time) {
        ...
        while (iter != iend) {
            ...
            auto val = GetGrid().GetValue(i, j, k);
            auto im = GetGrid().GetValue(i - 1, j, k);
            auto ip = GetGrid().GetValue(i + 1, j, k);
            auto jm = GetGrid().GetValue(i, j - 1, k);
            auto jp = GetGrid().GetValue(i, j + 1, k);
            auto km = GetGrid().GetValue(i, j, k - 1);
            auto kp = GetGrid().GetValue(i, j, k + 1);
            ...
            if (val > 0 && (im <= 0 || ip <= 0 || jm <= 0 || jp <= 0 || km <= 0 || kp <= 0)) {
                ...
                // interpolateDistance(val, im, ip, jm, jp, km, kp));
            } else if (val <= 0 && (im > 0 || ip > 0 || jm > 0 || jp > 0 || km > 0 || kp > 0)) {
                mAccepted[Coordinate(i, j, k, true)] =
                    InterpolateDistance(-val, -im, -ip, -jm, -jp, -km, -kp);
        ...
        while (iterAccepted != iendAccepted) {
            ...
            auto i = (*iterAccepted).first.i;
            auto j = (*iterAccepted).first.j;
            auto k = (*iterAccepted).first.k;
        ...
        while (!mHeap.isEmpty()) {
            ...
            auto i = node->coordinate.i;
            auto j = node->coordinate.j;
            auto k = node->coordinate.k;
...
protected:
    ...
    float MaxNormGradient() {
        ...
        while (iter != iend) {
            ...
            auto ddxc = mLS->DiffXpm(i, j, k);
            auto ddyc = mLS->DiffYpm(i, j, k);
            auto ddzc = mLS->DiffZpm(i, j, k);
            auto normgrad2 = ddxc * ddxc + ddyc * ddyc + ddzc * ddzc;
