...
public:
    ...
    virtual float ComputeTimestep() {
        ...
        // timestep t stable as given by the Courant-Friedrichs-Lewy (CFL) stability condition in Eq. 12 --> 
        // min(mdx + mdy + mdz) / |mF| --> grid spacing
    ...
    virtual float Evaluate(size_t i, size_t j, size_t k) {
        ...
        // instantiate the squared partial derivatives we'll evaluate with Godunov's scheme
        float ddx2, ddy2, ddz2;

        // Evaluate the squared partial derivatives at point (i,j,k) under speed function mF    
        LevelSetOperator::Godunov(i, j, k, mF, ddx2, ddy2, ddz2);

        // Compute the rate of change dphi/dt using the speed function mF --> Eq. 9b
        float rate = -1.0f * mF * std::sqrt(ddx2 + ddy2 + ddz2); 
