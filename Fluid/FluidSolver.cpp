    ...
    for (size_t i = 0; i < mVoxels.GetDimX(); i++) {
        for (size_t j = 0; j < mVoxels.GetDimY(); j++) {
            for (size_t k = 0; k < mVoxels.GetDimZ(); k++) {
                ...
                // Check if we are in fluid
                if (IsFluid(i, j, k)) {

                    // Convert the grid points to world space and save them in (x,y,z)
                    TransformGridToWorld(i, j, k, x, y, z);

                    // Get the initial velocity field at (i,j,k) --> i.e V1
                    glm::vec3 V1 = mVelocityField.GetValue(i, j, k);

                    // Get the external force field F at point (x,y,z)
                    glm::vec3 F = mExternalForces->GetValue(x, y, z);

                    // Compute the velocity field V2 after F has been applied to V1 --> Eq. 7
                    // (explicit euler)
                    glm::vec3 V2 = V1 + dt * F;

                    // Set the new velocity field at (i,j,k) to V2
    ...
    for (size_t i = 0; i < mVoxels.GetDimX(); i++) {
        for (size_t j = 0; j < mVoxels.GetDimY(); j++) {
            for (size_t k = 0; k < mVoxels.GetDimZ(); k++) {
                ...
                // Check we are in fluid
                if (IsFluid(i, j, k)) {

                    // Get the current velocity field at (i,j,k) --> it is V2 since we've applied F
                    // but no self-advection
                    glm::vec3 V2 = mVelocityField.GetValue(i, j, k);

                    // Check if we are next to a solid in x direction --> if neighbor is solid set
                    // velocity component to 0 since the Dirichlet boundary condition says that the
                    // fluid glues itself onto surface --> 0 fluid velocity
                    // --> psedo code on p. 5-6
                    if (IsSolid(i - 1, j, k) && V2.x < 0.f) {
                        V2.x = 0.f;
                    } else if (IsSolid(i + 1, j, k) && V2.x > 0.f) {
                        V2.x = 0.f;
                    }

                    // Do same checks in y- and z direction
                    if (IsSolid(i, j - 1, k) && V2.y < 0.f) {
                        V2.y = 0.f;
                    } else if (IsSolid(i, j + 1, k) && V2.y > 0.f) {
                        V2.y = 0.f;
                    }

                    if (IsSolid(i, j, k - 1) && V2.z < 0.f) {
                        V2.z = 0.f;
                    } else if (IsSolid(i, j, k + 1) && V2.z > 0.f) {
                        V2.z = 0.f;
                    }

                    // Assign this updated V2 to the velocity field
                    mVelocityField.SetValue(i, j, k, V2);
                }
            }
        }
        ...
        for (size_t j = 0; j < mVoxels.GetDimY(); j++) {
            for (size_t k = 0; k < mVoxels.GetDimZ(); k++) {
                ...
                if (IsFluid(i, j, k)) {
                    ...
                    // Equation 12 gives \nabla V_2 = \nabla^2q <--> b = \nabla^2q --> discretized version in eq. 13 ind is the
                    // index of the current cell at position (i,j,k) 
                    b[ind] = (mVelocityField.GetValue(i + 1, j, k).x - mVelocityField.GetValue(i - 1, j, k).x) / (2.f * mDx) +
                             (mVelocityField.GetValue(i, j + 1, k).y - mVelocityField.GetValue(i, j - 1, k).y) / (2.f * mDx) +
                             (mVelocityField.GetValue(i, j, k + 1).z - mVelocityField.GetValue(i, j, k - 1).z) / (2.f * mDx);
                    ...
                    // Look at neighbors to (i,j,k) in x, y and z direction --> neighbors are 1 if they are fluid too, like the center
                    // -6 in the center since we have 6 neighbors and we want to sum them up
                    std::vector<float> neighbors = {1.f, 1.f, 1.f, -6.f, 1.f, 1.f, 1.f};

                    // Now check if point is inside or outside the solid using eq. 16
                    if (IsSolid(i + 1, j, k)) { 
                        neighbors[0] = 0.f;  // i+1 is solid
                        neighbors[3] += 1.;  // add 1 to center if neighbor is solid
                    }
                    ...
                    // Make matrix A = \nabla^2 => A = (1/dx2) * neighbors --> eq. 16 (where dx2 = mDx^2)
                    A(ind, ind_ip) = neighbors[0] / dx2;
                    A(ind, ind_im) = neighbors[1] / dx2;
                    A(ind, ind_jp) = neighbors[2] / dx2;
                    A(ind, ind) = neighbors[3] / dx2;
                    A(ind, ind_jm) = neighbors[4] / dx2;
                    A(ind, ind_kp) = neighbors[5] / dx2;
                    A(ind, ind_km) = neighbors[6] / dx2;
                }
            }
        }
    }
        ...
        for (size_t j = 0; j < mVoxels.GetDimY(); j++) {
            for (size_t k = 0; k < mVoxels.GetDimZ(); k++) {
                ...
                if (IsFluid(i, j, k)) {
                    ...
                    // Central differencing of x = q in eq. 12 --> x has same # of elements as there are voxels (xDim*yDim*zDim)
                    // --> center scheme in eq.  but for x ir the gradients
                    glm::vec3 gradient_x = glm::vec3((x[ind_ip] - x[ind_im]) / (2.f * mDx),
                                                (x[ind_jp] - x[ind_jm]) / (2.f * mDx),
                                                (x[ind_kp] - x[ind_km]) / (2.f * mDx));

                    // Get the current velocity field at (i,j,k) --> it is V2 since we've applied F only
                    glm::vec3 V2 = mVelocityField.GetValue(i, j, k);

                    // Subtract the gradient of x from the velocity field to obtain v3 - ie obtain V3
                    mVelocityField.SetValue(i, j, k, V2 - gradient_x); 
                }
            }
        }
    }
