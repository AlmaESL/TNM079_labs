...
glm::vec3 LoopSubdivisionMesh::VertexRule(size_t vertexIndex) {
    ...
    // oneRing size = k = valence
    std::size_t k = oneRing.size();

    // compute angle beta
    float beta = Beta(k);

    // compute the new vertex position based on the weight beta, 1-k*beta
    glm::vec3 vtx = (1 - k * beta) * v(vertexIndex).pos;

    // compute weights for the neighboring vertices
    for (size_t index : oneRing) {
        vtx += beta * v(index).pos;
    }
...
glm::vec3 LoopSubdivisionMesh::EdgeRule(size_t edgeIndex) {
    ...
    // Get the halfedges for the edge
    HalfEdge& e0 = e(edgeIndex);
    HalfEdge& e1 = e(e0.pair);
    // Get the vertices for the halfedges
    HalfEdge& e2 = e(e0.prev);
    HalfEdge& e3 = e(e1.prev);

    // Get the vertex positions from the edges
    glm::vec3& v0 = v(e0.vert).pos;
    glm::vec3& v1 = v(e1.vert).pos;
    glm::vec3& v2 = v(e2.vert).pos;
    glm::vec3& v3 = v(e3.vert).pos;

    // Compute the position of the new vertex, vn = 3/8(v0) + 3/8v1 + 1/8v2 + 1/8v3 =
    // (3/8)(v0 + v1) + (1/8)(v2 + v3)
    glm::vec3 resPos = (3.0f / 8.0f) * (v0 + v1) + (1.0f / 8.0f) * (v2 + v3);
