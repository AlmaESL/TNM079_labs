...
public:
    LoopSubdivisionMesh(const HalfEdgeMesh& m, size_t s) : HalfEdgeMesh(m), mNumSubDivs(s) {}
...
protected:
    ...
    virtual std::vector<std::vector<glm::vec3>> Subdivide(size_t faceIndex);
    ...
    virtual glm::vec3 EdgeRule(size_t edgeIndex);
};
